...
$penerbit = mysqli_query($db, "SELECT buku.id, buku.penerbit FROM buku WHERE penerbit LIKE '%$key%' ORDER BY id DESC LIMIT {$page->awalData()},{$page->dataPerhalaman()}");
			...
			<tbody width="100%" cellspacing="10">
				...
				foreach ($penerbit as $penerbitnya):
					...
					<tr cellspacing="10">
						...
						<td>
							<button onclick="
								$('.popup').load('../Welcome/component/result/fraction_group.php?bukid=<?= $penerbitnya['id'] ?>&&usr=1&&pjm_id=5&&uls=1 #pop-up', ()=>{
									$('#pop-up').fadeIn(500);
								});
	...
	<div class="description">
		...
		<div class="pagination">
			<?php if ($page->halamanAktif() > 1): ?>
				...
				$('.isi-data').load(
					'component/result/penerbit.php?lim=<?= $page->dataPerhalaman() ?>&&page=<?= $page->halamanAktif() - 1 ?>&&key=<?= $key ?>'
				)">
