...
$write = mysqli_query($db, "SELECT buku.id, buku.penulis FROM buku ORDER BY id DESC LIMIT {$pagenation->dataPerhalaman()}");
		...
		<div class="data-indicator">
			<div class="data-entries">
				...
				<select id="selection" name="selection"
					onchange="
                    let value = $(this).val();
                    $('#isi-data').load('component/result/penulis.php?lim=' + value + '&&page=<?= $pagenation->halamanAktif() ?>&&key=' + $('#search').val())">
			...
			<div class="data-search">
                ...
                $('#isi-data').load(
                    'component/result/penulis.php?lim=' + $('#selection').val() + '&&page=<?= $pagenation->halamanAktif() ?>&&key=' + $(this).val()
                )">
			</div>
		</div>
		<!-- isi data -->
		<div class="isi-data" id="isi-data">
				...
				<table width="100%">
					...
					<tbody width="100%" cellspacing="10">
						...
						<tr cellspacing="10">
							...
							<td>
								<button onclick="
                                $('.popup').load('../Welcome/component/result/fraction_group.php?bukid=<?= $writer['id'] ?>&&usr=1&&pjm_id=5&&uls=1 #pop-up', ()=>{
                                    $('#pop-up').fadeIn(500);
                                });
			...
			<div class=" description">
				...
				<div class="pagination">
					...
					<button onclick="
                    $('.isi-data').load('component/result/penulis.php?lim=<?= $pagenation->dataPerhalaman() ?>&&page=<?= $pagenation->halamanAktif() + 1 ?>&&key=' + $('#search').val())'
                    )">
						Next
